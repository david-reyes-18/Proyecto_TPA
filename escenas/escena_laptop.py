...
class EscenaLaptop(EscenaBase):
    ...
    def dibujar(self, pantalla: pygame.Surface):
        pantalla.fill((15, 20, 40))
 
        # Título
        surf_titulo = self.fuente_titulo.render("💻  LAPTOP", False, (100, 200, 255))
        pantalla.blit(surf_titulo, surf_titulo.get_rect(centerx=pantalla.get_width()//2, y=80))
 
        # Contenido placeholder
        lineas = [
            "Aquí irá el desafío de la Laptop.",
            "",
            "[ ESC ] → volver al mapa",
        ]
        for i, linea in enumerate(lineas):
            surf = self.fuente_info.render(linea, False, (200, 220, 255))
